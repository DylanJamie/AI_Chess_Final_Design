...
NNUE_1_PATH = os.path.expanduser("~/Downloads/model_testing/carlsen500.nnue")
NNUE_1_NAME = "Carlsen 500 epochs"
...
def configure_engine(engine, nnue_path=None, elo=None):
    """
    Configures engine based on the specific options supported by your Stockfish build.
    """
    config = {
        "Hash": HASH_MB,
        "Threads": THREADS
    ...
    if nnue_path:
        # Your engine uses 'EvalFile' to load the net and 'EvalFileSmall' is also available,
        # but 'EvalFile' is the standard for the primary NNUE.
        config["EvalFile"] = os.path.abspath(os.path.expanduser(nnue_path))
    
    if elo:
        # These match the available options in your error message.
