...
# THIS LIST IS NEVER ADDED TO...? THATS WHY cleanup() DOESNT DO ANYTHING
ssh_connections = []
...
def open_ssh_connections_terminal(pi):
    ...
    # spawn the SSH session in a new terminal
    subprocess.Popen(["ptyxis", "--execute", ssh_cmd])
...
def cleanup(sig, frame):
    ...
    # since the ssh sessions are never appended to the "ssh_connections" list, this doesnt do anything.
    """
    ...
    # this does what the above block of code was supposed to do...
    for pi in PIS:
        user_host = f"{pi['username']}@{pi['host']}"
        cmd = f"pkill python && pkill -f LED_Program.py && pkill -f lcd_animation.py && pkill -f pi_chess_server_white.py"
        ssh_cmd = f"ssh -i {keypath} -t {user_host} '{cmd}'"
